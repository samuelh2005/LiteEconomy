...
public class EconomyService {
    ...
    public static CompletionStage<Boolean> completeTransaction(Transaction transaction) {
        ...
        return pending.completeWith(success);
    }

    private static boolean applyTransaction(Transaction transaction) {
        if (!canTransfer(transaction.getActor(), transaction.getFrom(), transaction.getAmount())) {
            return false;
        }

        BankAccount from = transaction.getFrom();
        BankAccount to = transaction.getTo();
